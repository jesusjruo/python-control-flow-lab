...
def calculate_dog_years():
    # Your control flow logic goes here
    dog_age_string = input("Enter Dog's age: " )
    dog_age = list(range(1 , int(dog_age_string) + 1 , 1))
    human_age = 0
    for num in dog_age:
        if(num == 1 or num == 2):
            human_age += 10
            ...
            human_age += 7
    print(f'The dog is {human_age} years old')
...
def weather_advice():
    ...
    if is_cold == "yes" and not is_raining == "yes":
        print("Wear a warm coat")
    if not is_cold == "yes" and is_raining == "yes":
        print("Carry an umbrella.")
    if not is_cold == "yes" and not is_raining == "yes":
...
def determine_season():
    ...
    if not month.isalpha() or month not in winter_months + spring_months + summer_months + fall_months:
